...
def generate_sparse_inputs(question_encoder, tensorizer, questions, query_token=None):
    max_length = 512
    pad_to_max = True
    ...
    def text_to_tensor(
        ...
        if pad_to_max and len(token_ids) < seq_len:
            token_ids = token_ids + [tensorizer.pad_token_id] * (seq_len - len(token_ids))
        if len(token_ids) >= seq_len:
            token_ids = token_ids[0:seq_len] if apply_max_len else token_ids
            token_ids[-1] = tensorizer.sep_token_id
    ...
    if query_token:
        ...
        if query_token == "[START_ENT]":
            ...
            batch_tensors = [text_to_tensor(" ".join([query_token, q])) for q in batch_questions]
    # elif isinstance(batch_questions[0], T):
    #     batch_tensors = [q for q in batch_questions]
    else:
        batch_tensors = [text_to_tensor(q) for q in batch_questions]

    q_ids_batch = torch.stack(batch_tensors, dim=0)
    q_seg_batch = torch.zeros_like(q_ids_batch)
    q_attn_mask = q_ids_batch != tensorizer.pad_token_id
    return {'input_ids': q_ids_batch.cuda(), 'token_type_ids': q_seg_batch.cuda(), 'attention_mask': q_attn_mask.cuda()}


def generate_inputs(tensorizer, questions, query_token=None):
    max_length = 512
    pad_to_max = True
    ...
    q_seg_batch = torch.zeros_like(q_ids_batch)
    q_attn_mask = q_ids_batch != tensorizer.pad_token_id
    return {'input_ids': q_ids_batch, 'token_type_ids': q_seg_batch, 'attention_mask': q_attn_mask}
...
def generate_question_vectors(
    ...
    # for j, batch_start in enumerate(range(0, n)):
    batch_questions = questions
    ...
    device = question_encoder.device
    ...
    if selector:
        ...
        # _, out, _ = question_encoder(q_ids_batch.to(device), q_seg_batch.to(device), q_attn_mask.to(device))
        ret = question_encoder(input_ids=q_ids_batch, token_type_ids=q_seg_batch, attention_mask=q_attn_mask)
        out = ret['last_hidden_state'][:, 0]
    ...
    print("query size:", len(questions))
...
class DenseRetriever(object):
    ...
    def generate_question_vectors(self, questions: List[str], query_token: str = None) -> T:
        ...
        bsz = self.batch_size
        # self.question_encoder.eval().cuda()
    ...
    def generate_context_vectors(self, questions: List[str], conetxt_encoder) -> T:
        bsz = self.batch_size
        # self.question_encoder.eval().cuda()
        return generate_question_vectors(
            conetxt_encoder,
