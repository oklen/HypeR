...
def find_and_process(dataset, output_dir, topn=-1):
    for file in os.listdir(output_dir):
        ...
        evaluate(dataset, output_dir + vec_path, epoch_count, topn)
        print(f"Evaluate done: {file}")

beir_dataset = {'msmarco','trec-covid','nfcorpus','bioasq','nq','hotpotqa','fiqa','arguana','webis-touche2020','cqadupstack','quora','dbpedia-entity','scidocs','fever','climate-fever','scifact','trec-covid-v2'}
def evaluate(dataset, vec_path, epoch_count, topn=-1):
    ...
    if topn == -1:
        commend = f"/relevance2-nfs/zefeng/anserini/target/appassembler/bin/SearchCollection -hits 100 -parallelism 95 \
                    -index /relevance2-nfs/zefeng/anserini/indexes/lucene-index.msmarco-real_kilt-passage-distill-splade \
                    -topicreader TsvString -topics {vec_path} \
                    -output {vec_path}-results-{dataset} -format trec \
                    -impact -pretokenized"
    else:
        commend = f"/relevance2-nfs/zefeng/anserini/target/appassembler/bin/SearchCollection -hits 100 -parallelism 95 \
                    -index /relevance2-nfs/zefeng/anserini/indexes/lucene-index.msmarco-real_kilt-passage-distill-splade-{topn} \
                    -topicreader TsvString -topics {vec_path} \
                    -output {vec_path}-results-{dataset} -format trec \
                    -impact -pretokenized"
    ...
    for line_info in lines:
        ...
        if wiki_id is None:
            wiki_id = -1
            print("Cound not find title in passage: {}, skip it.".format(t_passage[1]))
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--topn", type=int, default=-1, help="QPS Evaluation")
    parser.add_argument(
        "--output_dir",
        type=str,
        required=False,
    ...
    while True:
        find_and_process(args.dataset, args.output_dir, args.topn)
        time.sleep(5)
